...
import { plans } from "./components/planData"
...
export default function PricingPage() {
  ...
  const handleBuy = () => {
    if (selected.isEnterprise) {
      // redirect link approach via Link
      window.location.href = "/contact";
    } else {
      loadRazorpay(selected.name, selected.amount);
    }
  };
  ...
  return (
    ...
    href="/brochure.pdf"
    download
    className="mt-4 inline-block px-6 py-2 rounded-full text-white bg-gradient-to-r from-purple-400 to-purple-950 hover:from-blue-600 hover:to-blue-400 transition"
  >
    Download Brochure
  </a>
      </div>
      <div className="max-w-4xl mx-auto flex space-x-4 justify-center mb-8">
        {plans.map((p) => (
          <button
            key={p.name}
      ...
      <div className="max-w-4xl mx-auto">
        <PlanDetails plan={selected} onBuy={handleBuy} />
      </div>
    </div>
  );
}
